...
import pokemonoceanblue.battle.*;
...
public class AppManager {
    ...
    public static AppManager createInstance(int width, int height) {
        appManagerInstance = new AppManager(width, height);
        return appManagerInstance;
    }
    ...
    protected AppManager(int width, int height) {
        this.loadData();
        ...
        if (!canFly) {
            for (int i = 0; i < partyModel.team.size(); i++) {
                if (pokemonoceanblue.Type.typeIncludes(pokemonoceanblue.Type.FLYING, partyModel.team.get(i).types)
                        || pokemonoceanblue.Type.typeIncludes(pokemonoceanblue.Type.DRAGON,
                                partyModel.team.get(i).types)) {
                    canFly = true;
                    break;
                }
            }
    ...
    public void openParty(int currentPokemon, boolean returnSelection) {
        ...
        if (!(this.modelQueue.get(0) instanceof PartyModel)) {
    ...
    public void openController() {
        if (this.modelQueue.size() > 0) {
            ...
            if (newModel instanceof PokemonStorageModel) {
            ...
            else if (newModel instanceof OverworldModel) {
    ...
    /**
     * Returns to the preview screen For example, returns from the inventory screen
     * to either the overworld or battle If there are any new Pokemon / evolution
     * animations to show, this function will show them
     */
    public void exitCurrentView() {
        ...
        // return to a battle
        if (this.modelQueue.get(0) instanceof BattleModel) {
            BattleView battleView = new BattleView(this.battleModel, this.overworldModel.getBattleBackgroundId());
            viewManager.setView(battleView);
        }
        ...
        // return to inventory
        else if (this.modelQueue.get(0) instanceof InventoryModel) {
            InventoryView inventoryView = new InventoryView(inventoryModel);
            viewManager.setView(inventoryView);
        }
        // return to overworld
        else if (this.modelQueue.get(0) instanceof OverworldModel) {
            this.overworldModel.setWeather(this.weather);
    ...
    /**
     * Handles the catching of new Pokemon This constructor is used when the calling
     * class doesn't have access to pokedexModel so this function determines the
     * shiny chance
     * 
    ...
    /**
     * Add a Pokemon to your party if there is space, or PC otherwise, without
    ...
    public void update(List<Integer> keysDown) {
        ...
        if (viewManager.getCurrentView().equals("BattleView") && this.battleModel != null) {
            ...
            if (this.battleModel.isComplete() && this.partyModel.isDefeated()) {
                ...
                if (this.battleModel instanceof TrainerBattle) { // TODO: move some of this code into BattleModel subclasses
                    this.achievements
                            .forEach((obj) -> obj.incrementCounter("battleWin", 1, ((TrainerBattle)this.battleModel).trainerSpriteName));
                }
                ...
                if (battleResult.badgeIndex > -1 && !this.badges[battleResult.badgeIndex]
        ...
        // update the players position
            ...
            // check if player is entering a portal
                ...
                this.addModelQueue(newPokemonQueue.get(0));
                newPokemonQueue.remove(0);
            } else if (this.tournamentModel != null && overworldModel.conversation == null
                    && overworldModel.mapId % 100 == 0) {
            ...
            if (this.overworldModel.completeConversation >= 0) {
                this.quests.forEach((obj) -> obj.incrementCounter("conversation", 1,
                        String.valueOf(this.overworldModel.completeConversation)));
            ...
            if (this.currentController != null) {
                ...
                if (partyModel.isSummary && returnValue > -1) {
                    ...
                    if (this.battleModel != null) {
                        ...
                        // remove the Pokemon from the player's team if they were left in the day care,
                        // etc.
            ...
            if (this.currentController != null) {
                if (this.summaryModel.newMove != null) {
                    ...
                    this.modelQueue.remove(0);
                    if (this.modelQueue.get(0) instanceof PokemonStorageModel) {
    ...
    /**
     * don't add the quest if it's already in the list otherwise add the quest in
     * sorted order
     * 
        ...
        for (int i = 0; i < evolveQueue.length; i++) {
            if (evolveQueue[i]) {
                ...
                if (evolvedPokemonId != -1) {
                    ...
                    NewPokemonModel newPokemonModel = new NewPokemonModel(partyModel.team.get(i), partyModel,
                            evolvedPokemonId, i);
                    this.newPokemonQueue.add(newPokemonModel);
    ...
    /**
     * Checks if new daily quests should be loaded and then selects the daily quests
     * at random
     */
